<template>
  <q-page class="column">
    ...
    <div class="absolute q-ma-md">
      ...
      <q-btn color="white" text-color="black" @click="cange_camera()">
...
async function show_area_info(area_id) {
  alert.value = true
  const { introduce, name, image } = await get_attractions_card_info(area_id)
...
function cange_camera() {
  icon.value.classList.toggle('parallelogram');
  move_camera()
}
</script>
